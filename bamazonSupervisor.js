...
function departSales() {
  connection.query(
    "SELECT departments.department_id, products.department, departments.over_head_costs, SUM(product_sales) FROM products LEFT JOIN departments ON departments.department = products.department GROUP BY department",
    function(err, res) {
      if (err) throw err;
      var table = new Table({
        head: [
...
function addDepart() {
  inquirer
    ...
    .then(function(res) {
      connection.query(
        `INSERT INTO departments (department, over_head_costs) VALUES ('${res.newdepart}', '${res.newOver}')`,
